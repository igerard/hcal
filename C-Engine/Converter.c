...
static struct DateResult finish_up(long hday, long absolute, long hyear, long hmonth, long syear, long smonth,
                                   bool julianp);
...
long absolute_from_gregorian(long year, long month, long day)
{
	long xyear, day_number;
 ...
 * (Gregorian)
 */
long absolute_from_julian (long year, long month, long day)
{
  	long xyear, day_number;
 ...
 */
long absolute_from_hebrew (long year, long month, long day)
{
	long sum = day + hebrew_elapsed_days(year) - 1373429L;
	long i;
	if (month < 7) {
		long months = hebrew_months_in_year(year);
		for (i = 7; i <= months; i++)
...
void gregorian_from_absolute (long date, long *yearp, long *monthp, long *dayp)
{
	long year, month, day;
	for (year = date/366;
...
void julian_from_absolute (long date, long *yearp, long *monthp, long *dayp)
{
	long year, month, day;
	for (year = (date + 2)/366;
...
void hebrew_from_absolute(long date, long *yearp, long *monthp, long *dayp)
{
	long year, month, day, gyear, gmonth, gday, months;
...
long hebrew_month_length(long year, long month)
{
  switch (month) {
...
long secular_month_length(long year, long month, bool julianp)
{
	switch(month) {
...
bool gregorian_leap_year_p(long year)
{
...
bool julian_leap_year_p(long year)
{
	return ((year % 4) == 0);
}

/* Is it a leap year in the Jewish Calendar */
bool hebrew_leap_year_p(long year)
{
  	switch (year % 19) {
 ...
 */
#define MEMORY 5
long hebrew_elapsed_days(long year)
{
	static long saved_year[MEMORY] = {-1, -1, -1, -1, -1};
...
long hebrew_elapsed_days2(long year)
{
...
long hebrew_year_length(long year)
{
...
struct DateResult SecularToHebrewConversion(long syear, long smonth, long sday, bool julianp)
{
	long hyear, hmonth, hday;
	long absolute;
...
struct DateResult HebrewToSecularConversion(long hyear, long hmonth, long hday, bool julianp)
{
...
static struct DateResult finish_up(long hday, long absolute, long hyear, long hmonth, long syear, long smonth, bool julianp)
{
